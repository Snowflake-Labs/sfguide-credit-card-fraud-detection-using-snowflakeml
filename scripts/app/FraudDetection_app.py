...
#set = session.sql("UPDATE CC_FINS_DB.ANALYTICS.CC_APP_TBL SET TRANSACTION_DATE = DATEADD(day, -1, CURRENT_DATE) + (CAST(TRANSACTION_DATE AS TIMESTAMP) - CAST(TRANSACTION_DATE AS DATE));").collect()
...
def get_predictions():
    ...
    session.sql("USE ROLE SYSADMIN")
    final_df.write.mode("overwrite").save_as_table("inference_fd_table")
    #view = session.sql("CREATE OR REPLACE VIEW fraud_classification_input_view AS SELECT * FROM inference_fd_table ").collect()
    # Get predictions    
    result = session.sql("CREATE OR REPLACE TABLE predictions_creditcardfd AS SELECT *,fraud_classification_model!PREDICT(INPUT_DATA => object_construct(*)) as predictions from CC_FINS_DB.ANALYTICS.inference_fd_table").collect()
...
if st.button('Predict Fraudulent Transactions'):
    with st.spinner("Getting predictions..."):
        ...
        raw_data = session.sql("select * from CC_APP_TBL")
